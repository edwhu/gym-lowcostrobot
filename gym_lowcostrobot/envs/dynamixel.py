...
class Dynamixel:
    ...
    def set_operating_mode(self, motor_id: int, operating_mode: OperatingMode):
        # p_gain, dxl_comm_result, dxl_error = self.packetHandler.read2ByteTxRx(self.portHandler, 3, 84) # P Gain
        # i_gain, dxl_comm_result, dxl_error = self.packetHandler.read2ByteTxRx(self.portHandler, 3, 82) # I Gain
        # d_gain, dxl_comm_result, dxl_error = self.packetHandler.read2ByteTxRx(self.portHandler, 3, 80) # D Gain
        # mode, dxl_comm_result, dxl_error = self.packetHandler.read2ByteTxRx(self.portHandler, 3, 11) # operating mode
        # print(f"{motor_id}, {p_gain}, {i_gain}, {d_gain}, {mode}")
        dxl_comm_result, dxl_error = self.packetHandler.write2ByteTxRx(self.portHandler, motor_id,
