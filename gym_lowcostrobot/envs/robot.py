...
class Robot:
    ...
    def __init__(self, dynamixel, baudrate=1_000_000, servo_ids=[1, 2, 3, 4, 5, 6]):
        ...
        # print("commented out disable torque")
    ...
    def _set_position_control(self):
        self._disable_torque()
        for motor_id in self.servo_ids:
            # p_gain, dxl_comm_result, dxl_error = self.dynamixel.packetHandler.read2ByteTxRx(self.dynamixel.portHandler, 3, 84) # P Gain
            # i_gain, dxl_comm_result, dxl_error = self.dynamixel.packetHandler.read2ByteTxRx(self.dynamixel.portHandler, 3, 82) # I Gain
            # d_gain, dxl_comm_result, dxl_error = self.dynamixel.packetHandler.read2ByteTxRx(self.dynamixel.portHandler, 3, 80) # D Gain
            # print(f"{motor_id}, {p_gain}, {i_gain}, {d_gain}")
